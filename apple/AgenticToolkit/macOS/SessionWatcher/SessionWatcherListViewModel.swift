...
public final class SessionWatcherListViewModel: ObservableObject, @unchecked Sendable {
    ...
    public func summarizeSession(_ session: SessionWatcherSession) {
        SessionWatcherSummarizerDebugLog.shared.append("Manual summarize requested for \(session.sessionId)")
        guard let summarizer = sessionSummarizer else {
            SessionWatcherSummarizerDebugLog.shared.append("sessionSummarizer is nil on SessionWatcherListViewModel!")
            return
        }
        guard !summarizingSessionIds.contains(session.sessionId) else {
            SessionWatcherSummarizerDebugLog.shared.append("Already summarizing \(session.sessionId), skipping")
            return
        }
