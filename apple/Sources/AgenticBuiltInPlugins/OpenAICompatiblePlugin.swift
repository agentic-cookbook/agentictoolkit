...
public final class OpenAICompatiblePlugin: NSObject, AgenticLLMPlugin, @unchecked Sendable {
            ...
            Task {
                do {
                    ...
                    guard http.statusCode == 200 || http.statusCode == 201 else {
                        ...
                        throw PluginRequestError.httpError(http.statusCode, OpenAIPlugin.parseErrorMessage(from: body))
                    }

                    let reply = OpenAIPlugin.parseReply(from: data)
    ...
    public func settingsView() -> NSView? {
        // The base URL field is managed by AISettingsView which reads baseURL
        // from AISettingsViewModel. Return nil here — the framework handles it.
        nil
    }
    ...
    public func validateCredentials(_ credentials: PluginCredentials) async -> String? {
        ...
        do {
            ...
            let body = String(data: data, encoding: .utf8) ?? ""
            return OpenAIPlugin.parseErrorMessage(from: body) ?? "HTTP \(http.statusCode)"
        } catch {
