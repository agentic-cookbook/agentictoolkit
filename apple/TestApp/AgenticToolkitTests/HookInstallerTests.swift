...
final class HookInstallerTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("AgenticToolkitHookInstallerTests-\(UUID().uuidString)")
    ...
    func testAllHookEventTypesAreInstalled() throws {
        ...
        for eventType in HookInstaller.hookedEventTypes {
            ...
            // Verify the hook contains the AgenticToolkit marker
            ...
            XCTAssertTrue(
                ...
                "Command for \(eventType) missing AgenticToolkit marker"
            )
        }
    }
    ...
    func testExistingHooksArePreserved() throws {
        ...
        XCTAssertEqual(preToolUseGroups?.count, 2, "Should have existing hook + AgenticToolkit hook")
        ...
        // Second group should be the AgenticToolkit hook
    ...
    func testDetectsHooksAlreadyInstalledWithPartialInstall() throws {
        // Simulate a partial install: only one event type has a AgenticToolkit hook
        ...
        let installer = makeInstaller()
        let result = installer.installHooks()
        // Should detect that AgenticToolkit hooks exist even if not all event types are present
    ...
    func testHooksAlreadyInstalledReturnsFalseForNonAgenticToolkitHooks() {
        let installer = makeInstaller()
    ...
    func testHooksAlreadyInstalledReturnsTrueForAgenticToolkitHooks() {
        let installer = makeInstaller()
        let settings: [String: Any] = [
            "hooks": [
                "SessionStart": [
    ...
    func testUninstallRemovesAgenticToolkitHooks() throws {
        let installer = makeInstaller()
    ...
    func testUninstallPreservesNonAgenticToolkitHooks() throws {
        // Write settings with both a user hook and then install AgenticToolkit hooks
        let existingSettings: [String: Any] = [
            "hooks": [
                "PreToolUse": [
                    [
    ...
    func testHookCommandContainsMarker() {
        ...
        for eventType in HookInstaller.hookedEventTypes {
            ...
            XCTAssertTrue(
                ...
                "Command for \(eventType) should contain the AgenticToolkit marker"
            )
        }
    }
