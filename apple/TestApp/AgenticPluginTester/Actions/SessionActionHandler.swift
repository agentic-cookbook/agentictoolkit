...
final class SessionActionHandler {
    ...
    private func activateWarpSession(for session: Session) -> SessionActionResult {
        ...
        guard Self.isAccessibilityTrusted else {
            ...
            return .failure(.permissionDenied(
                "AgenticPluginTester needs Accessibility access to raise Warp windows. Grant access in System Settings.",
                pane: .accessibility
            ))
        }
    ...
    private func activateMatchingWindow(for session: Session) -> SessionActionResult {
        ...
        guard Self.isAccessibilityTrusted else {
            return .failure(.permissionDenied(
                "AgenticPluginTester needs Accessibility access to discover windows. Grant access in System Settings.",
                pane: .accessibility
            ))
        }
        ...
        for app in NSWorkspace.shared.runningApplications {
            ...
            for axWindow in axWindows {
                ...
                // +5 if the full cwd last two components match (e.g., "projects/AgenticPluginTester")
        ...
        // This handles multiple sessions in the same project (e.g., two AgenticPluginTester tabs).
        let best: Candidate
    ...
    private func sendNotification(for session: Session) -> SessionActionResult {
        ...
        content.title = "AgenticPluginTester: \(session.projectName)"
        content.body = "Session: \(session.sessionId)\nModel: \(session.model)\nStatus: \(session.status.rawValue)"
        content.sound = .default

        let request = UNNotificationRequest(
            identifier: "agentic-plugin-tester-click-\(session.sessionId)-\(Date().timeIntervalSince1970)",
            content: content,
            trigger: nil
        )
    ...
    private func appleScriptPermissionError(_ error: NSDictionary, appName: String) -> SessionActionError? {
        ...
        if errorNumber == -1743 || errorNumber == -1744
            ...
            return .permissionDenied(
                "AgenticPluginTester needs permission to control \(appName). Grant access in System Settings > Privacy & Security > Automation.",
                pane: .automation
            )
        }
        return nil
