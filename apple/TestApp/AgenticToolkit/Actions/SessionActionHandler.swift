...
final class SessionActionHandler {
    ...
    private func activateWarpSession(for session: Session) -> SessionActionResult {
        ...
        guard Self.isAccessibilityTrusted else {
            ...
            return .failure(.permissionDenied(
                "AgenticToolkit needs Accessibility access to raise Warp windows. Grant access in System Settings.",
                pane: .accessibility
            ))
        }
    ...
    private func activateMatchingWindow(for session: Session) -> SessionActionResult {
        ...
        guard Self.isAccessibilityTrusted else {
            return .failure(.permissionDenied(
                "AgenticToolkit needs Accessibility access to discover windows. Grant access in System Settings.",
                pane: .accessibility
            ))
        }
        ...
        for app in NSWorkspace.shared.runningApplications {
            ...
            for axWindow in axWindows {
                ...
                // +5 if the full cwd last two components match (e.g., "projects/AgenticToolkit")
        ...
        // This handles multiple sessions in the same project (e.g., two AgenticToolkit tabs).
        let best: Candidate
    ...
    private func sendNotification(for session: Session) -> SessionActionResult {
        ...
        content.title = "AgenticToolkit: \(session.projectName)"
    ...
    private func appleScriptPermissionError(_ error: NSDictionary, appName: String) -> SessionActionError? {
        ...
        if errorNumber == -1743 || errorNumber == -1744
            ...
            return .permissionDenied(
                "AgenticToolkit needs permission to control \(appName). Grant access in System Settings > Privacy & Security > Automation.",
                pane: .automation
            )
        }
        return nil
