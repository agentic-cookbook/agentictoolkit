...
/// AgenticToolkit can ingest and display session activity in real time.
///
/// The installer merges AgenticToolkit hooks with any existing user hooks (append, not overwrite)
/// and detects whether its hooks are already present to avoid duplicates.
final class HookInstaller {

    // MARK: - Constants

    /// Marker embedded in every AgenticToolkit hook command so we can identify our hooks later.
    static let hookMarker = "# agentic-plugin-tester-hook"

    /// The directory where hook event files are written.
    static let defaultDropDirectory = "~/.claude/session-events"

    /// The event types that AgenticToolkit installs hooks for.
    ...
    /// Installs AgenticToolkit hooks into the Claude Code settings file.
    /// - Returns: The result of the installation attempt.
    func installHooks() -> InstallResult {
        do {
            var settings = try loadSettings()

            // Check if AgenticToolkit hooks are already installed
            ...
            for eventType in Self.hookedEventTypes {
                hooks = appendAgenticToolkitHook(eventType: eventType, to: hooks)
            }
    ...
    /// Uninstalls all AgenticToolkit hooks from the Claude Code settings file.
            ...
            for eventType in Self.hookedEventTypes {
                if var matcherGroups = hooks[eventType] as? [[String: Any]] {
                    ...
                    matcherGroups = matcherGroups.filter { group in
                        !isAgenticToolkitMatcherGroup(group)
                    }
    ...
    /// Checks whether AgenticToolkit hooks are already installed in the given settings dictionary.
    func hooksAlreadyInstalled(in settings: [String: Any]) -> Bool {
        guard let hooks = settings["hooks"] as? [String: Any] else {
            return false
        }

        // Check if at least one event type has a AgenticToolkit hook
        for eventType in Self.hookedEventTypes {
            if let matcherGroups = hooks[eventType] as? [[String: Any]] {
                for group in matcherGroups {
                    if isAgenticToolkitMatcherGroup(group) {
                        return true
                    }
                }
            }
    ...
    /// Checks whether a matcher group contains a AgenticToolkit hook (identified by the marker comment).
    private func isAgenticToolkitMatcherGroup(_ group: [String: Any]) -> Bool {
    ...
    /// Appends a AgenticToolkit hook for the given event type to the hooks dictionary.
    /// Preserves any existing matcher groups for the same event type.
    private func appendAgenticToolkitHook(eventType: String, to hooks: [String: Any]) -> [String: Any] {
        var result = hooks
        var matcherGroups = hooks[eventType] as? [[String: Any]] ?? []

        let matcherGroup = makeAgenticToolkitMatcherGroup(eventType: eventType)
    ...
    /// Creates a matcher group dictionary for a AgenticToolkit hook.
    private func makeAgenticToolkitMatcherGroup(eventType: String) -> [String: Any] {
    ...
    /// 2. Uses `jq` to extract relevant fields and construct a AgenticToolkit event JSON
    ...
    func makeHookCommand(eventType: String) -> String {
        // Build a jq filter that constructs the AgenticToolkit event JSON from stdin.
        ...
        // - The marker comment is on the first line so we can detect AgenticToolkit hooks
        var command = """
        \(Self.hookMarker)
        INPUT=$(cat)
    ...
    /// Builds a jq filter expression that constructs a AgenticToolkit event JSON object
