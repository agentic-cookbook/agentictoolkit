...
final class SettingsViewModel: ObservableObject {
    ...
    @Published var launchAtLogin: Bool {
        didSet {
            ...
            do {
                ...
                NSLog("AgenticToolkit: Failed to \(launchAtLogin ? "enable" : "disable") launch at login: \(error.localizedDescription)")
    ...
    func loadFromDatabase() {
        do {
            ...
            // Migrate API key from SQLite to Keychain if needed
            if !KeychainHelper.exists(forKey: AISettingsViewModel.apiKeyKeychainKey) {
                if let sqliteKey = settings["ai_api_key"], !sqliteKey.isEmpty {
                    KeychainHelper.set(sqliteKey, forKey: AISettingsViewModel.apiKeyKeychainKey)
                    try? databaseManager.deleteSetting(key: "ai_api_key")
            ...
            NSLog("AgenticToolkit: Failed to load settings: \(error.localizedDescription)")
        }
    }
    ...
    private func saveSetting(key: String, value: String) {
        ...
        do {
            ...
            NSLog("AgenticToolkit: Failed to save setting '\(key)': \(error.localizedDescription)")
        }
    }
