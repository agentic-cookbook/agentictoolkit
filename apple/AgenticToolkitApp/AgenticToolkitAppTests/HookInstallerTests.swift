...
final class HookInstallerTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("AgenticPluginTesterHookInstallerTests-\(UUID().uuidString)")
    ...
    func testAllHookEventTypesAreInstalled() throws {
        ...
        for eventType in HookInstaller.hookedEventTypes {
            ...
            // Verify the hook contains the AgenticPluginTester marker
            ...
            XCTAssertTrue(
                command?.contains(HookInstaller.hookMarker) == true,
                "Command for \(eventType) missing AgenticPluginTester marker"
            )
        }
    }
    ...
    func testExistingHooksArePreserved() throws {
        ...
        XCTAssertEqual(preToolUseGroups?.count, 2, "Should have existing hook + AgenticPluginTester hook")
        ...
        // Second group should be the AgenticPluginTester hook
        let hookGroup = preToolUseGroups?.last
        let hookList2 = hookGroup?["hooks"] as? [[String: Any]]
        let hookCommand = hookList2?.first?["command"] as? String
        XCTAssertTrue(hookCommand?.contains(HookInstaller.hookMarker) == true)
    }
    ...
    func testDetectsHooksAlreadyInstalledWithPartialInstall() throws {
        // Simulate a partial install: only one event type has a AgenticPluginTester hook
        let partialSettings: [String: Any] = [
            "hooks": [
                "SessionStart": [
                    [
                        "hooks": [
                            ["type": "command", "command": "\(HookInstaller.hookMarker)\necho 'test'"]
                        ]
                    ]
                ]
            ]
        ...
        let installer = makeInstaller()
        let result = installer.installHooks()
        // Should detect that AgenticPluginTester hooks exist even if not all event types are present
    ...
    func testHooksAlreadyInstalledReturnsFalseForNonAgenticPluginTesterHooks() {
        let installer = makeInstaller()
        let settings: [String: Any] = [
            "hooks": [
                "PreToolUse": [
                    [
                        "hooks": [
                            ["type": "command", "command": "echo 'not agentic'"]
                        ]
                    ]
                ]
            ]
    ...
    func testHooksAlreadyInstalledReturnsTrueForAgenticPluginTesterHooks() {
        let installer = makeInstaller()
        let settings: [String: Any] = [
            "hooks": [
                "SessionStart": [
    ...
    func testUninstallRemovesAgenticPluginTesterHooks() throws {
        let installer = makeInstaller()
    ...
    func testUninstallPreservesNonAgenticPluginTesterHooks() throws {
        // Write settings with both a user hook and then install AgenticPluginTester hooks
        let existingSettings: [String: Any] = [
            "hooks": [
                "PreToolUse": [
                    [
    ...
    func testHookCommandContainsMarker() {
        ...
        for eventType in HookInstaller.hookedEventTypes {
            ...
            XCTAssertTrue(
                command.contains(HookInstaller.hookMarker),
                "Command for \(eventType) should contain the AgenticPluginTester marker"
            )
        }
    }
