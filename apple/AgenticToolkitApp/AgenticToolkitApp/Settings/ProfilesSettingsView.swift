...
/// Profiles settings: split layout with profile list (left) and detail (right).
/// Built-in profiles are read-only but can be duplicated. Custom profiles are editable and deletable.
final class ProfilesSettingsView: NSView, NSTableViewDataSource, NSTableViewDelegate, NSSplitViewDelegate {

    private var profiles: [AppProfile] = AppProfile.builtInProfiles()
    ...
    private func selectInitialProfile() {
        let activeID = UserDefaults.standard.string(forKey: "app.activeProfileID")
            ?? AppProfile.defaultProfileID
    ...
    private func makeDetailPane() -> NSView {
        ...
        for appearance in ProfileAppearance.allCases {
    ...
    func tableViewSelectionDidChange(_ notification: Notification) {
        ...
        UserDefaults.standard.set(profiles[row].id.uuidString, forKey: "app.activeProfileID")
        NotificationCenter.default.post(name: Self.profileDidChangeNotification, object: nil)
    }

    static let profileDidChangeNotification = Notification.Name("AppProfileDidChange")
    ...
    private func updateColorPreview(profile: AppProfile) {
    ...
    private func makeTerminalPreview(profile: AppProfile) -> NSView {
        let stack = NSStackView()
        stack.orientation = .vertical
        stack.alignment = .leading
        stack.spacing = 2
    ...
    @objc private func appearanceChanged() {
        guard let index = selectedProfileIndex,
              let appearance = appearancePopUp.selectedItem?.representedObject as? ProfileAppearance else { return }
    ...
    @objc private func cursorChanged() {
        guard let index = selectedProfileIndex,
              let style = cursorPopUp.selectedItem?.representedObject as? CursorStyle else { return }
