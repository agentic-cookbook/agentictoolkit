...
final class SessionsDatabaseManager {
    ...
    /// Returns the default database path: ~/Library/Application Support/AgenticPluginTester/data.db
    static func defaultDatabasePath() throws -> String {
        let appSupport = FileManager.default.urls(
            for: .applicationSupportDirectory,
            in: .userDomainMask
        ).first!
        let appDir = appSupport.appendingPathComponent("AgenticPluginTester")
        ...
        return appDir.appendingPathComponent("data.db").path
    }
