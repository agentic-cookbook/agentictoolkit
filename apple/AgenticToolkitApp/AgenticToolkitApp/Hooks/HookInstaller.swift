...
/// AgenticPluginTester can ingest and display session activity in real time.
///
/// The installer merges AgenticPluginTester hooks with any existing user hooks (append, not overwrite)
/// and detects whether its hooks are already present to avoid duplicates.
final class HookInstaller {

    // MARK: - Constants

    /// Marker embedded in every AgenticPluginTester hook command so we can identify our hooks later.
    ...
    /// The event types that AgenticPluginTester installs hooks for.
    ...
    /// Installs AgenticPluginTester hooks into the Claude Code settings file.
    ...
    func installHooks() -> InstallResult {
        do {
            ...
            for eventType in Self.hookedEventTypes {
                hooks = appendAgenticPluginTesterHook(eventType: eventType, to: hooks)
            }
    ...
    /// Uninstalls all AgenticPluginTester hooks from the Claude Code settings file.
            ...
            for eventType in Self.hookedEventTypes {
                if var matcherGroups = hooks[eventType] as? [[String: Any]] {
                    ...
                    matcherGroups = matcherGroups.filter { group in
                        !isAgenticPluginTesterMatcherGroup(group)
                    }
    ...
    /// Checks whether AgenticPluginTester hooks are already installed in the given settings dictionary.
    ...
    /// Checks whether a matcher group contains a AgenticPluginTester hook (identified by the marker comment).
    private func isAgenticPluginTesterMatcherGroup(_ group: [String: Any]) -> Bool {
    ...
    /// Appends a AgenticPluginTester hook for the given event type to the hooks dictionary.
    /// Preserves any existing matcher groups for the same event type.
    private func appendAgenticPluginTesterHook(eventType: String, to hooks: [String: Any]) -> [String: Any] {
        var result = hooks
        var matcherGroups = hooks[eventType] as? [[String: Any]] ?? []

        let matcherGroup = makeAgenticPluginTesterMatcherGroup(eventType: eventType)
    ...
    /// Creates a matcher group dictionary for a AgenticPluginTester hook.
    private func makeAgenticPluginTesterMatcherGroup(eventType: String) -> [String: Any] {
    ...
    /// 2. Uses `jq` to extract relevant fields and construct a AgenticPluginTester event JSON
    ...
    func makeHookCommand(eventType: String) -> String {
        // Build a jq filter that constructs the AgenticPluginTester event JSON from stdin.
        ...
        // - The marker comment is on the first line so we can detect AgenticPluginTester hooks
        var command = """
        \(Self.hookMarker)
        INPUT=$(cat)
    ...
    /// Builds a jq filter expression that constructs a AgenticPluginTester event JSON object
