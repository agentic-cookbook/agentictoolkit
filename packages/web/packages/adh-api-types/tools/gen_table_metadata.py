...
(GET / PUT / DELETE). The dumped spec (frontend/src/main/api/openapi.json) carries
...
def resolve_spec(argv: list[str] | None = None) -> Path:
    ...
    if not raw:
        raise SystemExit(
            ...
            "  ADH_OPENAPI_SPEC=<adh>/frontend/src/main/api/openapi.json "
...
// edit by hand. Source: adh's frontend/src/main/api/openapi.json, which is NOT in
// this repo — the spec is an explicit input, so regenerate with a path to an adh
// checkout:
//   pnpm --filter @agentic-toolkit/adh-api-types gen:crud-metadata -- <adh>/frontend/src/main/api/openapi.json
