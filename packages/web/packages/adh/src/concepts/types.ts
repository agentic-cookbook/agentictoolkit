...
// The concept taxonomy — the single source of truth behind the shared concept
// graph (the landing pages), the `/details/<topic>` routes, and the details-page
// nav. This file is PURE DATA + TYPES (no React) so it can be imported from
// server components, route `generateStaticParams`/`generateMetadata`, and the
// client header alike.
//
// Editing guide: the tree literal lives in `tree.ts`. Keep node order STABLE
// (the radial graph layout is a deterministic function of focus + child order,
// and that determinism is what lets us server-render the graph and hydrate it
// without a mismatch). Never introduce Date/Math.random into anything the layout
// reads.
...
export type DetailSection =
  ...
  | { kind: 'diagram'; heading?: string; ascii: string }
...
// ── Structure / content split (the two authored layers) ─────────────────────
//
// A ConceptNode is the ASSEMBLED view. The authored sources are two layers:
//  • a locale-agnostic STRUCTURE (the shape — ids, hierarchy, routing, theming),
//  • a per-locale CONTENT catalog (the copy — labels, blurbs, detail bodies).
// `assembleTree(structure, content, locale)` merges them back into a ConceptTree,
// so every existing consumer keeps the same shape. This split is what the content
// editor edits and what makes the copy translatable. See `assemble.ts`.
