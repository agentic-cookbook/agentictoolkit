...
export const BAG_KEY_REQUIRED_MESSAGE = "A key is required.";
export const duplicateBagKeyMessage = (key: string) => `A bag named “${key}” already exists.`;
 ...
 */
export function isBagFormDirty(form: BagFormState, initial: BagFormState): boolean {
  return (
    form.key.trim() !== initial.key.trim() ||
    form.valueText !== initial.valueText ||
 ...
 * the bug.
 */
export function bagFormBlockedReason(form: BagFormState, ctx: BagFormContext): string | null {
  try {
