 ...
 * and nothing else.
 *
 * This site is the one that PAID for the convergence. Its `/home` was the forum board — public
 * content with a quiet sign-in nudge, not a workspace at all — and it was the only site in the
 * family with no workspace concept. `/home` is the family's redirect signal, so the board moved
 * to `/forum`, which is what the header and every breadcrumb already called it. Nothing redirects
 * `/home` to it: a 308 there would be this site answering the signal differently from the other
 * 37, which is the exact shape of divergence the template exists to make impossible.
 *
 ...
 */
export const communityHome = defineSiteHome({
  ...
  parse: noSubPath,
  render: () => <SiteHomePlaceholder siteId="community" />,
});
