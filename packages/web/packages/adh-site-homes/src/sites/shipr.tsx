...
import { ShiprHome } from "@agentic-toolkit/shipr";
 ...
 *
 * THIS IS THE WHOLE SITE-SIDE IMPLEMENTATION, and it is four lines because the console's mount
 * is shipr's UI and lives with it (`ShiprHome`, in @agentic-toolkit/shipr). What is left here is
 * the two things only a host knows: that this site has no grammar below the workspace, and how a
 * request from it is signed.
 *
 * `authedFetch` rather than `fetch` — it carries the session and turns a 401 into one silent
 * refresh-and-retry rather than a console full of red — is passed IN rather than imported by the
 * console, because the shipr package deliberately takes its fetcher from the host (see `Fetcher`
 * in shipr/client). This package IS that host: knowing how an adh request is signed is exactly
 * what it is for, and it is the reason the console can be mounted somewhere that signs differently.
 *
 * Auth: both mounts sit under a HomeGate layout.
 */
export const shiprHome = defineSiteHome({
  ...
  parse: noSubPath,
  // The whole workspace ROW, not just its slug: the console's breadcrumb starts at the workspace,
  // and its NAME is what an operator calls it — the slug is an address.
  render: ({ workspaceSlug, workspace }) => (
    <ShiprHome fetcher={authedFetch} workspaceSlug={workspaceSlug} rootLabel={workspace.name} />
  ),
});
