 ...
 * CreateGameAction) navigates rather than setting state, because `creating` above is itself
 * derived from this parse, not from a component's `useState` — there is nothing for the button
 * to set directly. So the feature reads the reserved segment off the URL instead, a URL like
 * any other: reachable by a bookmark, a typed address, or a refresh. A search param would have
 * needed `useSearchParams`, which opts a route into client-side rendering at build time, to buy
 * nothing this does not already give (both are deep-linkable and both survive a refresh).
 */
