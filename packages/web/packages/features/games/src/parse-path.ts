 ...
 * so the whole grammar stays in this one pure function: the Create Game button lives in
 * the workspace bar, in a different subtree from the feature, so it cannot signal the
 * dialog through React state — it navigates, and the feature reads the URL. A search
 * param would have needed `useSearchParams`, which opts a route into client-side
 * rendering at build time, to buy nothing this does not already give (both are
 * deep-linkable and both survive a refresh).
 */
